...
@section('title', 'Create Movie - CineBook')
                ...
                <form action="{{ route('movies.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('poster') is-invalid @enderror" 
                               id="poster" name="poster" value="{{ old('poster') }}" placeholder="https://example.com/poster.jpg">
                        @error('poster')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="url" class="form-control @error('trailer_link') is-invalid @enderror" 
                               id="trailer_link" name="trailer_link" value="{{ old('trailer_link') }}" placeholder="https://youtube.com/watch?v=...">
                        @error('trailer_link')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('synopsis') is-invalid @enderror" 
                                  id="synopsis" name="synopsis" rows="4">{{ old('synopsis') }}</textarea>
                        @error('synopsis')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control @error('rating_average') is-invalid @enderror" 
                               id="rating_average" name="rating_average" value="{{ old('rating_average', 0) }}" min="0" max="5">
